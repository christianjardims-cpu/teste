...
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "16.2_github_embedded_weather":
    st.session_state.clear()  
    st.session_state.versao_layout = "16.2_github_embedded_weather"
...
# -----------------------------------------------------------------------------
# 3. TELEMETRIA METEOROLÓGICA REAL-TIME (OPEN-METEO) - MULTIFUNCIONAL EMBUTIDA
# -----------------------------------------------------------------------------
@st.cache_data(ttl=300)
def obter_dados_meteorologicos_puros_v11():
    try:
        url = "https://api.open-meteo.com/v1/forecast?latitude=-30.1139&longitude=-51.3250&current=temperature_2m,apparent_temperature,rain,wind_speed_10m,uv_index&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=America/Sao_Paulo"
        ...
        temp_atual = f"{int(current.get('temperature_2m', 14))}°C"
        sensacao = f"{int(current.get('apparent_temperature', 14))}°C"
        vento = f"{int(current.get('wind_speed_10m', 0))} km/h"
        chuva = f"{current.get('rain', 0.0)} mm"
        uv = f"{current.get('uv_index', 0.0)}"
        
        url_legado = "https://api.open-meteo.com/v1/forecast?latitude=-30.1139&longitude=-51.3250&current_weather=true&timezone=America/Sao_Paulo"
        res_legado = requests.get(url_legado, timeout=5).json().get("current_weather", {})
        status_atual = wmo_codes.get(res_legado.get("weathercode"), "☁️ Encoberto")
        ...
        return temp_atual, status_atual, cronograma, sensacao, vento, chuva, uv
    except Exception:
        cronograma_fallback = [
        ...
        return "14°C", "☁️ Encoberto", cronograma_fallback, "14°C", "12 km/h", "0.0 mm", "1.0"

temp_real, status_real, dados_clima, sensacao_real, vento_real, chuva_real, uv_real = obter_dados_meteorologicos_puros_v11()
...
# -----------------------------------------------------------------------------
# 6. HEADER DESIGN CONTEMPORÂNEO (MÉTRICAS EMBUTIDAS NA BARRA E ALINHADAS)
# -----------------------------------------------------------------------------
col_tit1, col_tit2, col_signature = st.columns([1.8, 1.6, 0.6])
with col_tit1:
...
with col_tit2:
    st.markdown(f"""
        <div class='ios-clock-widget'>
            ...
            <div class='ios-progress-label' style='display:flex; justify-content:space-between; align-items:center;'>
                <span>⏱️ Progresso da Jornada Ativa</span>
            </div>
            
            <div style='position: relative; background: #21262d; border-radius: 6px; border: 1px solid #30363d; padding: 2px;'>
                <div class='ios-progress-container' style='border: none; margin-bottom: 0; height: 8px;'>
                    <div class='ios-progress-bar' style='width: {pct_jornada}%;'></div>
                </div>
                <div style='display: flex; gap: 6px; justify-content: space-between; padding: 4px 6px 2px 6px; font-size: 0.68rem; font-family: "JetBrains Mono", monospace; color: #8b949e;'>
                    <span>💨 {vento_real}</span>
                    <span>🌧️ {chuva_real}</span>
                    <span>🌡️ ST: {sensacao_real}</span>
                    <span>☀️ UV: {uv_real}</span>
                </div>
            </div>
            
            <div class='ios-weather-row' style='display:flex; justify-content:space-between; align-items:center; font-size:0.78rem; margin-top:2px; border-top: 1px solid #21262d; padding-top: 6px;'>
                <span style='color:#8b949e;'>📍 Guaíba - RS <span style='color:#30363d; margin: 0 4px;'>|</span> <span style='color:#58a6ff; font-weight:600;'>{status_real.split(' ')[0]} {temp_real}</span></span>
                <span style='font-size: 0.7rem; color: #8b949e; font-family: "JetBrains Mono", monospace;'>
                    ST {sensacao_real} • 💨 {vento_real}
                </span>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
# 7. FILTROS E RENDERIZAÇÃO CORRIGIDA COM MUDANÇA DE COR INSTANTÂNEA
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 8. SISTEMA DE ABAS EM ALTA DENSIDADE
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    # -------------------------------------------------------------------------
    # NOVA ABA: VISÃO GERAL DA FÁBRICA (CONSOLIDADO COMPLETO)
    # -------------------------------------------------------------------------
    with aba_fabrica:
        ...
        with col_f2:
            st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Volume de Trabalho por Área</p>", unsafe_allow_html=True)
            if not df_fabrica_unicas.empty:
                area_counts_fabrica = df_fabrica_unicas["area"].value_counts().reset_index()
                fig_f2 = px.bar(area_counts_fabrica, x="area", y="count", text_auto=True, color="area", color_discrete_sequence=px.colors.qualitative.Muted)
                fig_f2.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(t=10,b=10,l=10,r=10), showlegend=False, xaxis_title="Áreas", yaxis_title="Quantidade de Ordens")
                st.plotly_chart(fig_f2, use_container_width=True, key="graph_fabrica_areas")
                
        st.markdown("<div style='margin: 15px 0; border-bottom: 1px solid #30363d;'></div>", unsafe_allow_html=True)
        st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Acompanhamento de Ordens Ativas (Fábrica)</p>", unsafe_allow_html=True)
        render_cards_operacionais(df_geral, "fabrica_completo")

    # -------------------------------------------------------------------------
    # ABA ORIGINAL: MÉTRICAS MACRO
    # -------------------------------------------------------------------------
    with aba_geral:
    ...
    # -------------------------------------------------------------------------
    # ABA ORIGINAL: LINHA POR DISCIPLINA
    # -------------------------------------------------------------------------
    with aba_disc:
