...
<style>
    ...
    .gemini-day-active { color: #58a6ff; font-weight: 600;
        font-size: 0.88rem; }
    .gemini-badge-today { 
        font-size: 0.7rem;
    ...
    .gemini-temp-text { font-weight: 600; color: #f0f6fc; font-size: 0.88rem;
    ...
    .ios-clock-top { display: flex; justify-content: space-between;
        align-items: center; }
    ...
    .kpi-value { font-size: 2.2rem; font-weight: 700; color: #f0f6fc;
        font-family: 'JetBrains Mono', monospace; margin-bottom: 4px; }
    .kpi-label { font-size: 0.75rem; color: #8b949e; text-transform: uppercase; font-weight: 600;
...
def salvar_ou_atualizar_registro_db(id_registro, novo_status, novo_comentario, ordem_nome=""):
    with sqlite3.connect(DB_NOME) as conn:
        ...
        conn.commit()
    # Avisa o ecossistema global que os dados em cache precisam de atualização em background
...
def render_cards_operacionais(sub_df, unique_prefix):
    ...
    for _, row in sub_df.iterrows():
        ...
        # FIX DEFINITIVO: Força o HTML do Card a adotar na mesma hora o valor do Widget antes de ler a base lenta
...
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    # Adicionada a nova aba 'Visão Geral da Fábrica' como primeira opção
