...
# -----------------------------------------------------------------------------
# 1. CONFIGURAÇÕES INICIAIS E DESIGN CORE (GEMINI X GITHUB PRESTIGE PREMIUM)
# -----------------------------------------------------------------------------
st.set_page_config(
...
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "16.2_global_modern_colors":
    st.session_state.clear()  
    st.session_state.versao_layout = "16.2_global_modern_colors"

# Paleta de Cores Corporativa Avançada (UI Moderna)
COLOR_MAP = {
    "Realizada": "#2ea44f",
    "Pendente": "#f85149",
...
DISCIPLINA_CORES = {
    "Mecânica": "#388bfd",       # Cyber Blue
    "Elétrica": "#8957e5",       # Tech Purple
    "Instrumentação": "#347d39"  # Emerald Green
}

# Paleta para Gráficos de Área e Comparativos (Cores Modernas e de Alto Contraste)
...
<style>
    ...
    .corp-title-side { font-size: 1.1rem; font-weight: 700; color: #f0f6fc; letter-spacing: 1px; margin: 0; }
    .corp-author-side { font-size: 0.72rem; color: #8b949e; margin-top: 2px; font-family: 'JetBrains Mono', monospace; line-height: 1.3; }
</style>
...
with st.sidebar:
    ...
    # Rodapé Globalizado e Assinatura Dev realocada para a Barra Lateral
    st.markdown(f"""
        <div class='sidebar-footer-container'>
            <center style='font-size:0.75rem; color:#8b949e; font-weight:500;'>🏭 Hub de Performance Industrial • CMPC Guaíba</center>
...
# -----------------------------------------------------------------------------
col_tit1, col_tit2 = st.columns([2.6, 1.4])
with col_tit1:
    ...
    st.markdown("<p style='color: #8b949e; margin: 0; font-size: 1rem; font-weight: 400;'>Sistema Integrado de Confiabilidade • Unidade Industrial Guaíba</p>", unsafe_allow_html=True)
with col_tit2:
    st.markdown(f"""
...
# -----------------------------------------------------------------------------
# 8. SISTEMA DE ABAS EM ALTA DENSIDADE (REESTRUTURADO)
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    # Atualização das nomenclaturas das abas e acréscimo da Visão Geral Corporativa
    ...
    # -------------------------------------------------------------------------
    # NOVA ABA: VISÃO GERAL DA FÁBRICA (AGREGADO CORPORATIVO)
    # -------------------------------------------------------------------------
    with aba_global:
        st.markdown("<h3 style='font-size:1.2rem; font-weight:500; margin-bottom:12px;'>Métricas Consolidadas da Unidade Guaíba</h3>", unsafe_allow_html=True)
        ...
        st.markdown(f"""
            <div class='kpi-container'>
                ...
                <div class='kpi-card'><div class='kpi-value' style='color:#58a6ff'>{len(lista_areas_disponiveis)}</div><div class='kpi-label'>Áreas Operacionais Monitoradas</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#7ee787'>{aderencia_g:.1f}%</div><div class='kpi-label'>Aderência Global de Manutenção</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#ff7b72'>{total_os_g - realizadas_g}</div><div class='kpi-label'>Backlog Ativo de Campo</div></div>
            </div>
        """, unsafe_allow_html=True)
        
        col_glob1, col_glob2 = st.columns([1.8, 2.2])
        with col_glob1:
            st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Composição Global de Status</p>", unsafe_allow_html=True)
            ...
            fig_g1.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(t=10,b=10,l=10,r=10), showlegend=True)
            st.plotly_chart(fig_g1, use_container_width=True, key="graph_global_pie_status")
            
        with col_glob2:
            st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Performance de Conclusão por Setor Fabril</p>", unsafe_allow_html=True)
            
            # Cálculo de aderência por área
            desempenho_areas = []
            ...
            fig_g2.update_layout(
                ...
                margin=dict(t=10,b=10,l=10,r=10), xaxis=dict(range=[0, 100], gridcolor='#21262d'),
                yaxis=dict(title=None)
            )
            fig_g2.update_traces(textposition="inside", marker_line_color="transparent")
            st.plotly_chart(fig_g2, use_container_width=True, key="graph_global_bar_areas")

    # -------------------------------------------------------------------------
    # ABA: VISÃO POR ÁREA (MÉTRICAS MACRO ANTERIOR)
    # -------------------------------------------------------------------------
    with aba_geral:
