...
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "19.3_github_gemini_global_refactor":
    st.session_state.clear()  
    st.session_state.versao_layout = "19.3_github_gemini_global_refactor"
...
with st.sidebar:
    ...
    # AJUSTADO: Assinatura do Dev Christian Jardim realocada de forma fixa para o rodapé da Sidebar
    st.markdown("""
...
# -----------------------------------------------------------------------------
# 6. HEADER DESIGN CONTEMPORÂNEO (GLOBAL REFACTOR)
# -----------------------------------------------------------------------------
...
with col_tit1:
    ...
    # AJUSTADO: Subtítulo globalizado para cobrir a fábrica como um todo
...
with col_tit2:
    # AJUSTADO: O balão da semana agora está alinhado na extremidade direita do card
    st.markdown(f"""
...
# -----------------------------------------------------------------------------
# 8. SISTEMA ESTRUTURAL DE ABAS (4 ABAS ORIGINAIS)
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    with aba_geral:
        ...
        with col_g2:
            ...
            if not df_geral_unicas.empty:
                ...
                fig_pizza_areas.update_traces(textinfo="percent+label", textfont=dict(family="JetBrains Mono", size=11))
        ...
        if not df_geral_unicas.empty:
            ...
            fig_barras_horizontal.update_layout(
                **layout_comum_github,
                ...
                height=240, showlegend=False, barmode='group'
            )
    ...
    with aba_exec:
        ...
        lista_executantes = ["Selecione the professional..."] + sorted(list(df_foco["executante"].dropna().unique()))
        exec_sel = st.selectbox("Filtrar por técnico de campo:", lista_executantes, key="c_exec_v19", label_visibility="collapsed")
        
        if exec_sel != "Selecione the professional...":
    ...
    # AJUSTADO: Rodapé com nome mais global e abrangente para a planta inteira
