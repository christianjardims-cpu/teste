...
<style>
    ...
    .side-corp-author { font-size: 0.72rem; color: #8b949e; margin-top: 3px; font-family: 'JetBrains Mono', monospace; line-height: 1.3; }
</style>
...
# -----------------------------------------------------------------------------
# 5. SIDEBAR: FLUID DESIGN WITH FOOTER SIGNATURE
# -----------------------------------------------------------------------------
with st.sidebar:
    ...
    # ASSINATURA GLOBALIZADA DO DEV MOVIDA PARA A BASE DA BARRA LATERAL
    st.markdown("""
...
# -----------------------------------------------------------------------------
# 8. SISTEMA DE ABAS EM ALTA DENSIDADE (GLOBALIZADO)
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    with aba_fabrica:
        ...
        with col_f2:
            st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Volume de Trabalho por Área (Todas as Áreas)</p>", unsafe_allow_html=True)
            if not df_fabrica_unicas.empty:
                area_counts_fabrica = df_fabrica_unicas["area"].value_counts().reset_index()
                area_counts_fabrica.columns = ["área", "quantidade"]
                area_counts_fabrica = area_counts_fabrica.sort_values(by="quantidade", ascending=False)
                ...
                fig_f2.update_layout(
                    ...
                    xaxis_title="Setores / Áreas Cadastradas", 
                    yaxis_title="Quantidade de Ordens (OMS)"
                )
    ...
    # -------------------------------------------------------------------------
    # ABA 2: VISÃO POR ÁREA (METRICAS MACRO POR ÁREA ESPECÍFICA)
    # -------------------------------------------------------------------------
    with aba_geral:
