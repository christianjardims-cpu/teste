...
# ---- AJUSTE DE PERSISTÊNCIA APÓS F5 NA URL ----
url_params = st.query_params
aba_salva = url_params.get("aba_atual", "Geral")

if "aba_atual" not in st.session_state:
    st.session_state.aba_atual = aba_salva

def atualizar_url_aba():
    st.query_params["aba_atual"] = st.session_state.aba_atual
# -----------------------------------------------
    ...
    for _, row in sub_df.iterrows():
        ...
        with col_rad:
            st.segmented_control(
                ...
                args=(id_reg, unique_prefix, ordem)
            )
        with col_input:
            if status_atual in ["Pendente", "Necessita Reprogramação"]:
                ...
                with col_sel:
                    st.selectbox(
                        "Justificativa",
                        ["Selecione motivo..."] + motivos,
                        index=(motivos.index(motivo_inicial) + 1 if motivo_inicial in motivos else 0),
                        key=f"mot_{unique_prefix}_{id_reg}",
                        label_visibility="collapsed",
                        on_change=tratar_mudanca_status,
                        args=(id_reg, unique_prefix, ordem)
                    )
                with col_det:
                    st.text_input(
                        "Detalhes adicionais...",
                        value=detalhe_inicial,
                        key=f"det_{unique_prefix}_{id_reg}",
                        placeholder="Detalhes adicionais...",
                        label_visibility="collapsed",
                        on_change=tratar_mudanca_status,
                        args=(id_reg, unique_prefix, ordem)
                    )
        st.markdown("<div style='margin: 14px 0; border-bottom: 1px dashed #21262d;'></div>", unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# 8. RENDERIZAÇÃO DAS ABAS PRINCIPAIS COM SUPORTE A NAVEGAÇÃO PERSISTENTE
# -----------------------------------------------------------------------------
lista_abas = ["Geral", "Elétrica", "Instrumentação", "Mecânica"]
index_inicial = lista_abas.index(st.session_state.aba_atual) if st.session_state.aba_atual in lista_abas else 0

aba_selecionada = st.segmented_control(
    "Menu de Disciplinas",
    options=lista_abas,
    default=st.session_state.aba_atual,
    key="aba_atual",
    on_change=atualizar_url_aba,
    label_visibility="collapsed"
)

# Renderiza com base no parâmetro lido ou selecionado
if aba_selecionada == "Geral":
    if st.session_state.db_data is not None and not st.session_state.db_data.empty:
        df_limpo = st.session_state.db_data.drop_duplicates(subset=["ordem"])
        
        totais = len(df_limpo)
        realizadas = len(df_limpo[df_limpo["status_execucao"] == "Realizada"])
        pendentes = len(df_limpo[df_limpo["status_execucao"] == "Pendente"])
        reprogs = len(df_limpo[df_limpo["status_execucao"] == "Necessita Reprogramação"])
        
        aderencia = (realizadas / totais * 100) if totais > 0 else 100.0
        
        st.markdown(f"""
            <div class='kpi-container'>
                <div class='kpi-card'><div class='kpi-value'>{totais}</div><div class='kpi-label'>📋 Total Escopo</div></div>
                <div class='kpi-card' style='border-bottom: 3px solid #2ea44f;'><div class='kpi-value' style='color:#2ea44f;'>{realizadas}</div><div class='kpi-label'>✅ Realizadas</div></div>
                <div class='kpi-card' style='border-bottom: 3px solid #f85149;'><div class='kpi-value' style='color:#f85149;'>{pendentes}</div><div class='kpi-label'>⏳ Pendentes</div></div>
                <div class='kpi-card' style='border-bottom: 3px solid #db6d28;'><div class='kpi-value' style='color:#db6d28;'>{reprogs}</div><div class='kpi-label'>🔄 Reprogramadas</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#58a6ff;'>{aderencia:.1f}%</div><div class='kpi-label'>📈 Taxa de Aderência</div></div>
            </div>
        """, unsafe_allow_html=True)
        
        col_g1, col_g2 = st.columns(2)
        with col_g1:
            contagem_status = df_limpo["status_execucao"].value_counts().reset_index()
            contagem_status.columns = ["Status", "Quantidade"]
            fig_pizza = px.pie(
                contagem_status, names="Status", values="Quantidade",
                color="Status", color_discrete_map=COLOR_MAP,
                hole=0.4, title="Distribuição do Status das Atividades"
            )
            fig_pizza.update_layout(margin=dict(t=40, b=0, l=0, r=0), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#c9d1d9")
            st.plotly_chart(fig_pizza, use_container_width=True)
            
        with col_g2:
            df_limpo["horas_num"] = df_limpo["tempo_execucao"].apply(extrair_horas)
            tempo_disc = df_limpo.groupby("disciplina")["horas_num"].sum().reset_index()
            fig_barra = px.bar(
                tempo_disc, x="disciplina", y="horas_num",
                labels={"disciplina": "Disciplina", "horas_num": "Carga Horária Acumulada (h)"},
                title="Carga de Trabalho Estimada por Disciplina",
                color="disciplina", color_discrete_map=DISCIPLINA_CORES
            )
            fig_barra.update_layout(margin=dict(t=40, b=0, l=0, r=0), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font_color="#c9d1d9")
            st.plotly_chart(fig_barra, use_container_width=True)
            
        st.markdown("### 🗓️ Cronograma Integrado do Turno")
        df_limpo["data_parsed"] = pd.to_datetime(df_limpo["data_inicio"], errors="coerce")
        df_limpo["dia_nome"] = df_limpo["data_parsed"].dt.day_name()
        st.markdown("<br>", unsafe_allow_html=True)
        
        if df_limpo["data_parsed"].isna().all():
            render_cards_operacionais(df_limpo, "geral_full_list")
        else:
            for eng_day, pt_day in dias_mapeamento.items():
                df_dia = df_limpo[df_limpo["dia_nome"] == eng_day].copy()
                if not df_dia.empty:
                    qtd_unicas = df_dia["ordem"].nunique()
                    expandir_se_hoje = (pt_day.split("-")[0] == hoje_nome_pt)
                    with st.expander(f"📅 {pt_day} — ({qtd_unicas} Atividades na Planta)", expanded=expandir_se_hoje):
                        render_cards_operacionais(df_dia, f"geral_crd_v19_{eng_day}")
    else:
        st.markdown("""
        <div style='border: 1px dashed #30363d; padding: 40px; border-radius: 6px; text-align: center; margin-top:20px;'>
            <p style='color: #8b949e; font-size:1.1rem; margin:0;'>Nenhum escopo carregado no SQLite local para a semana ativa.</p>
            <p style='color: #58a6ff; font-size:0.85rem; margin-top:8px;'>Insira a Chave Operacional na barra lateral para importar planilhas.</p>
        </div>
        """, unsafe_allow_html=True)

else:
    # LÓGICA FILTRADA POR DISCIPLINA ESPECÍFICA (ELÉTRICA / INSTRUMENTAÇÃO / MECÂNICA)
    disc_sel = aba_selecionada
    if st.session_state.db_data is not None and not st.session_state.db_data.empty:
        df_disc = st.session_state.db_data[st.session_state.db_data["disciplina"] == disc_sel].copy()
        
        if df_disc.empty:
            st.markdown(f"""
            <div style='border: 1px dashed #30363d; padding: 40px; border-radius: 6px; text-align: center; margin-top:20px;'>
                <p style='color: #8b949e; font-size:1.1rem; margin:0;'>Nenhuma ordem vinculada à disciplina de <b>{disc_sel}</b> nesta planilha.</p>
            </div>
            """, unsafe_allow_html=True)
        else:
            df_disc = df_disc.drop_duplicates(subset=["ordem"])
            totais_d = len(df_disc)
            realizadas_d = len(df_disc[df_disc["status_execucao"] == "Realizada"])
            pendentes_d = len(df_disc[df_disc["status_execucao"] == "Pendente"])
            reprogs_d = len(df_disc[df_disc["status_execucao"] == "Necessita Reprogramação"])
            aderencia_d = (realizadas_d / totais_d * 100) if totais_d > 0 else 100.0
            
            st.markdown(f"""
                <div class='kpi-container'>
                    <div class='kpi-card'><div class='kpi-value'>{totais_d}</div><div class='kpi-label'>📋 Escopo {disc_sel}</div></div>
                    <div class='kpi-card' style='border-bottom: 3px solid #2ea44f;'><div class='kpi-value' style='color:#2ea44f;'>{realizadas_d}</div><div class='kpi-label'>Realizadas</div></div>
                    <div class='kpi-card' style='border-bottom: 3px solid #f85149;'><div class='kpi-value' style='color:#f85149;'>{pendentes_d}</div><div class='kpi-label'>Pendentes</div></div>
                    <div class='kpi-card' style='border-bottom: 3px solid #db6d28;'><div class='kpi-value' style='color:#db6d28;'>{reprogs_d}</div><div class='kpi-label'>Reprogramadas</div></div>
                    <div class='kpi-card'><div class='kpi-value' style='color:#58a6ff;'>{aderencia_d:.1f}%</div><div class='kpi-label'>Aderência Técnica</div></div>
                </div>
            """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style='border: 1px dashed #30363d; padding: 40px; border-radius: 6px; text-align: center; margin-top:20px;'>
        <p style='color: #8b949e; font-size:1.1rem; margin:0;'>Selecione uma opção válida acima.</p>
    </div>
    """, unsafe_allow_html=True)