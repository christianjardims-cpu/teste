...
# -----------------------------------------------------------------------------
# 1. CONFIGURAÇÕES INICIAIS E DESIGN CORE (DESIGN MODERNO INDUSTRIAL)
# -----------------------------------------------------------------------------
st.set_page_config(
...
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "16.3_global_modern_colors":
    st.session_state.clear()  
    st.session_state.versao_layout = "16.3_global_modern_colors"
...
# Paleta Avançada por Disciplina (Cores Modernas de Alto Contraste)
DISCIPLINA_CORES = {
    "Mecânica": "#388bfd",       # Cyber Blue
    "Elétrica": "#8957e5",       # Tech Purple
    "Instrumentação": "#347d39"  # Emerald Tech
}

# Paleta Temática Complementar para Gráficos e Barras de Carga
PALETA_GRAFICOS_MODERNA = ["#388bfd", "#58a6ff", "#bc8cff", "#ff7b72", "#7ee787", "#f2cc60", "#6e7681"]

# -----------------------------------------------------------------------------
# ESTILIZAÇÃO INTERNA AVANÇADA (CSS HIGH-FIDELITY DARK MODE UI)
# -----------------------------------------------------------------------------
st.markdown("""
<style>
...
with st.sidebar:
    ...
    # Rodapé Globalizado e Realocação Estrutural da Assinatura Dev
    st.markdown(f"""
...
# -----------------------------------------------------------------------------
# 8. SISTEMA DE ABAS EM ALTA DENSIDADE (REESTRUTURADO E INTEGRADO)
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    # Nomenclatura Globalizada e Moderna das Abas
    ...
    with aba_global:
        ...
        with col_glob2:
            ...
            if not df_global_unicas.empty:
                ...
                # CORREÇÃO APLICADA: Substituído o valor inválido textposition/color por marker_line_width=0
                fig_g2.update_traces(textposition="inside", marker_line_width=0)
