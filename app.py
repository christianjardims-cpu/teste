...
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "19.2_github_gemini_inverted_charts":
    st.session_state.clear()  
    st.session_state.versao_layout = "19.2_github_gemini_inverted_charts"
    ...
    st.markdown(f"""
        <div class='ios-clock-widget'>
            <div class='ios-clock-top'>
                ...
                <span class='ios-date-badge'>{hoje_nome_pt} • {data_hoje_formatada} • Semana {semana_ano}</span>
            </div>
        ...
        with col_input:
            if status_atual in ["Pendente", "Necessita Reprogramação"]:
                ...
                if ":" in comentario_atual:
                    ...
                    if partes[0] in motifs: # fallback correction
...
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    with aba_geral:
        ...
        with col_g2:
            # INVERTIDO: O antigo terceiro gráfico (Volume de OMS estilo pizza) agora assume a segunda posição
        ...
        # INVERTIDO: O gráfico de Avanço Percentual por Área (em barras horizontais com cores dedicadas) agora fica abaixo na terceira posição
