...
with st.sidebar:
    ...
    st.markdown(f"""
        <div class='github-feed-box'>
            ...
            <div class='feed-item'><span class='feed-tag'>[ALERT]</span> Restrições climáticas updated em tempo real via telemetria.</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    # RODAPÉ DA ABA LATERAL ATUALIZADO CONFORME PEDIDO
    st.markdown("""
...
# -----------------------------------------------------------------------------
# 6. HEADER DESIGN CONTEMPORÂNEO (TÍTULO E SUBTÍTULO ATUALIZADOS)
# -----------------------------------------------------------------------------
...
with col_tit1:
    ...
    st.markdown("<p style='color: #8b949e; margin: 0; font-size: 1rem;'>Monitoramento de Performance e Confiabilidade Global de Aderencia</p>", unsafe_allow_html=True)
with col_tit2:
    st.markdown(f"""
...
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    # -------------------------------------------------------------------------
    with aba_exec:
        st.markdown("<h3 style='font-size:1.2rem; font-weight:500; margin-bottom:12px;'>Alocação Técnico por Executante</h3>", unsafe_allow_html=True)
    ...
    # RODAPÉ PRINCIPAL ATUALIZADO CONFORME PEDIDO
    st.markdown("<div style='margin-top:20px; font-size:0.75rem; color:#8b949e;'><center>Sistema Integrado de Visualização • Visão Global Planta Guaíba</center></div>", unsafe_allow_html=True)
else:
    st.markdown("""
