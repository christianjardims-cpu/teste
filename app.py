...
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "19.5_emoji_refactor":
    st.session_state.clear()  
    st.session_state.versao_layout = "19.5_emoji_refactor"
...
# -----------------------------------------------------------------------------
# INTERACTIVE SURFACE CSS ANIMATIONS (GEMINI STYLING)
# -----------------------------------------------------------------------------
st.markdown("""
<style>
    ...
    /* ESTILIZAÇÃO DO BARÔMETRO DE RISCO MULTIFUNCIONAL */
    .weather-risk-bar {
        width: 100%;
        height: 6px;
        border-radius: 3px;
    ...
    .risk-low { background-color: #2ea44f; }
    .risk-medium { background-color: #db6d28; }
    .risk-high { background-color: #f85149; animation: pulse 1.5s infinite ease-in-out; }
    ...
    .ios-date-badge { background: #21262d; color: #c9d1d9; font-size: 0.75rem; font-weight: 600; padding: 4px 10px; border-radius: 6px;
        border: 1px solid #30363d; }
    ...
    code { font-family: 'JetBrains Mono', monospace; color: #ff7b72; background: #21262d; padding: 2px 6px; border-radius: 4px;
        font-size: 0.85rem; border: 1px solid #30363d; }

    .kpi-container { display: flex; gap: 1rem; margin-bottom: 1.5rem; margin-top: 1rem;
        flex-wrap: wrap; }
    .kpi-card { 
        background: #161b22;
        border: 1px solid #30363d; 
    ...
    /* TELEMETRIA WIDGET ESTILO INDUSTRIAL */
    .telemetry-box {
        background: #161b22;
        border: 1px solid #30363d;
        border-radius: 6px;
    ...
    .corporate-signature-sidebar { 
        margin-top: 30px; 
        padding-top: 15px;
        border-top: 1px solid #30363d; 
    }
    .corp-title-sidebar { font-size: 1.1rem; font-weight: 700; color: #f0f6fc; letter-spacing: 0.5px; margin: 0; }
    .corp-author-sidebar { font-size: 0.72rem; color: #8b949e; margin-top: 2px; font-family: 'JetBrains Mono', monospace; line-height: 1.3; }
</style>
...
# -----------------------------------------------------------------------------
# 3. TELEMETRIA METEOROLÓGICA REAL-TIME INTEGRADA (OPEN-METEO)
# -----------------------------------------------------------------------------
@st.cache_data(ttl=300)
def obter_dados_meteorologicos_puros_v12():
    try:
        # Adicionado os parâmetros current_weather, apparent_temperature, precipitation, windspeed_10m e uv_index
        url = "https://api.open-meteo.com/v1/forecast?latitude=-30.1139&longitude=-51.3250&current=apparent_temperature,precipitation,wind_speed_10m,uv_index&daily=weathercode,temperature_2m_max,temperature_2m_min&timezone=America/Sao_Paulo"
        ...
        # Coleta das novas variáveis de telemetria imediata
        temp_atual = f"{int(current_data.get('apparent_temperature', 14))}°C"
        status_atual = wmo_codes.get(current_data.get("weather_code"), "☁️ Encoberto")
        vento = float(current_data.get("wind_speed_10m", 12.0))
        chuva = float(current_data.get("precipitation", 0.0))
        sensacao = f"{int(current_data.get('apparent_temperature', 14))}°C"
        uv_val = float(current_data.get("uv_index", 1.0))
        
        if uv_val <= 2: uv_txt = "Baixo"
        elif uv_val <= 5: uv_txt = "Moderado"
        elif uv_val <= 7: uv_txt = "Alto"
        else: uv_txt = "Crítico"
        ...
        return temp_atual, status_atual, cronograma, vento, chuva, sensacao, uv_txt
    except Exception:
        cronograma_fallback = [
        ...
        return "14°C", "☁️ Encoberto", cronograma_fallback, 12.0, 0.0, "14°C", "Baixo"

temp_real, status_real, dados_clima, vento_real, chuva_real, sensacao_real, uv_real = obter_dados_meteorologicos_puros_v12()

# Lógica Unificada de Risco de Campo
if vento_real > 30.0 or chuva_real > 2.0:
    risco_classe = "risk-high"
    risco_txt = "⚠️ Risco Crítico em Áreas Externas"
elif vento_real >= 15.0 or chuva_real > 0.0:
    risco_classe = "risk-medium"
    risco_txt = "⚡ Alerta para Trabalhos em Altura"
else:
    risco_classe = "risk-low"
    risco_txt = "✅ Condições Ideais para Campo"
...
# -----------------------------------------------------------------------------
# 5. SIDEBAR: FLUID INTERACTION SURFACE
# -----------------------------------------------------------------------------
with st.sidebar:
    ...
    # SEÇÃO DA TELEMETRIA - TRANSFORMAÇÃO DA BARRA EM FORMATO INTELIGENTE UNIFICADO
    st.markdown("<p style='font-size:0.7rem; color:#8b949e; font-weight:600; letter-spacing:0.8px; text-transform:uppercase; margin-bottom:4px;'>Telemetria Climática Semanal</p>", unsafe_allow_html=True)
    
    # 1. A Barra Dinâmica que muda de comportamento por cor e pulsação
    st.markdown(f"<div class='weather-risk-bar {risco_classe}' title='{risco_txt}'></div>", unsafe_allow_html=True)
    
    # 2. Exibição unificada dos Badges de métricas físicas em tempo real
    st.markdown(f"""
        <div class='weather-metrics-container'>
            <div class='weather-badge' title='Velocidade do Vento Atual'>🌬️ {vento_real:.1f} km/h</div>
            <div class='weather-badge' title='Precipitação acumulada'>🌧️ {chuva_real:.1f} mm</div>
            <div class='weather-badge' title='Sensação térmica imediata'>🌡️ {sensacao_real}</div>
            <div class='weather-badge' title='Índice de radiação ultravioleta'>☀️ UV: {uv_real}</div>
        </div>
        <div style='font-size:0.75rem; color:#8b949e; margin-bottom:14px; margin-top:-6px; font-weight:500; padding-left:2px;'>Status: <span style='color:#f0f6fc;'>{risco_txt}</span></div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
        <div class='corporate-signature-sidebar'>
            <p class='corp-title-sidebar'>CMPC • Instrumentação Energia</p>
            <p class='corp-author-sidebar'>Design & Development<br><b>Christian Jardim 2026</b></p>
        </div>
    """, unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
col_tit1, col_tit2 = st.columns([1.9, 1.1])
with col_tit1:
    st.markdown("<h1 style='font-weight: 600; font-size: 2.2rem; margin-bottom: 4px; font-family: \"SF Pro Display\",sans-serif;'>📊 Painel de Acompanhamento • Programação Semanal</h1>", unsafe_allow_html=True)
    st.markdown("<p style='color: #8b949e; margin: 0; font-size: 1rem;'>Monitoramento de Performance e Confiabilidade Global de Aderência</p>", unsafe_allow_html=True)
with col_tit2:
    st.markdown(f"""
        <div class='ios-clock-widget'>
            <div class='ios-clock-top'>
                ...
                <div style='display: flex; gap: 6px; align-items: center;'>
                    <span class='ios-date-badge'>{hoje_nome_pt} • {data_hoje_formatada}</span>
                    <span class='ios-date-badge' style='background: rgba(56, 139, 253, 0.15); color: #58a6ff; border-color: rgba(56, 139, 253, 0.4);'>Semana {semana_ano}</span>
                </div>
            </div>
...
# -----------------------------------------------------------------------------
# 7. FILTROS E RENDERIZAÇÃO DE CARDS FRAGMENTADOS
# -----------------------------------------------------------------------------
...
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    aba_geral, aba_area, aba_exec, aba_disc = st.tabs([
        "🏭 Visão Geral da Fábrica", 
        "🛠️ Visão por Área", 
        "👷 Visão por Executante", 
        "⚙️ Visão por Disciplina"
    ])
    ...
    layout_comum_github = dict(
        ...
        font=dict(family="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", size=11, color="#8b949e"),
        margin=dict(t=15, b=15, l=15, r=15)
    )

    # -------------------------------------------------------------------------
    # ABA 1: 🏭 VISÃO GERAL DA FÁBRICA
    # -------------------------------------------------------------------------
    with aba_geral:
        st.markdown("<h3 style='font-size:1.15rem; font-weight:500; margin-bottom:12px; color:#f0f6fc;'>Indicadores Globais de Performance</h3>", unsafe_allow_html=True)
        ...
        st.markdown(f"""
            <div class='kpi-container'>
                <div class='kpi-card'><div class='kpi-value'>{total_os_f}</div><div class='kpi-label'>Ordens Totais Planta</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#2ea44f'>{aderencia_f:.1f}%</div><div class='kpi-label'>Aderência Geral</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#f85149'>{total_os_f - realizadas_f}</div><div class='kpi-label'>Backlog Ativo</div></div>
            </div>
        """, unsafe_allow_html=True)
        ...
        with col_g1:
            ...
            if not df_geral_unicas.empty:
                ...
                st.plotly_chart(fig1, use_container_width=True, key="pie_global_status_v19")
         
        with col_g2:
            st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Volume de Ordens de Trabalho (OMS) por Área Fabril</p>", unsafe_allow_html=True)
            if not df_geral_unicas.empty:
                df_contagem_areas = df_geral_unicas["area"].value_counts().reset_index()
                df_contagem_areas.columns = ["Área", "Quantidade"]
                 
                fig_pizza_areas = px.pie(
                    df_contagem_areas, values="Quantidade", names="Área", hole=0.5,
                    color_discrete_sequence=PALETA_GRAFICOS_MODERNA
                )
                fig_pizza_areas.update_layout(**layout_comum_github)
                fig_pizza_areas.update_traces(
                    textinfo="value+percent", 
                    hovertemplate="<b>%{label}</b><br>Quantidade: %{value} OMS<br>Percentual: %{percent}<extra></extra>",
                    textfont=dict(family="JetBrains Mono", size=11)
                )
                st.plotly_chart(fig_pizza_areas, use_container_width=True, key="graph_pizza_oms_por_area_v19")

        st.markdown("<div style='margin: 15px 0; border-bottom: 1px solid #30363d;'></div>", unsafe_allow_html=True)
        
        st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Avanço Percentual Individual por Área</p>", unsafe_allow_html=True)
        if not df_geral_unicas.empty:
            fig_barras_horizontal = go.Figure()
            
            for idx, area_item in enumerate(lista_areas_disponiveis):
                df_area_item = df_geral_unicas[df_geral_unicas["area"].str.strip() == area_item]
                tot_a = len(df_area_item)
                rec_a = len(df_area_item[df_area_item["status_execucao"] == "Realizada"])
                pct_a = (rec_a / tot_a * 100) if tot_a > 0 else 0.0
                
                cor_barra = PALETA_GRAFICOS_MODERNA[idx % len(PALETA_GRAFICOS_MODERNA)]
                
                fig_barras_horizontal.add_trace(go.Bar(
                    y=[area_item], x=[pct_a], name=area_item, orientation='h',
                    marker=dict(color=cor_barra),
                    text=[f"{pct_a:.1f}% ({rec_a}/{tot_a} OMS)"], textposition='inside'
                ))
            
            altura_dinamica = max(260, len(lista_areas_disponiveis) * 32)
            
            fig_barras_horizontal.update_layout(
                **layout_comum_github,
                xaxis=dict(range=[0, 100], gridcolor='#21262d', title="Percentual Concluído por Setor"),
                yaxis=dict(autorange="reversed"),
                height=altura_dinamica, 
                showlegend=False, 
                barmode='group'
            )
            st.plotly_chart(fig_barras_horizontal, use_container_width=True, key="graph_v19_bar_area_advance")

    # -------------------------------------------------------------------------
    # ABA 2: 🛠️ VISÃO POR ÁREA
    # -------------------------------------------------------------------------
    with aba_area:
        area_sel_geral = st.selectbox("Selecione a Área de Foco:", lista_areas_disponiveis, key="area_macro_v19_back")
        ...
        st.markdown(f"""
            <div class='kpi-container'>
                ...
                <div class='kpi-card'><div class='kpi-value' style='color:#2ea44f'>{aderencia:.1f}%</div><div class='kpi-label'>Aderência Local</div></div>
                <div class='kpi-card'><div class='kpi-value' style='color:#f85149'>{total_os - realizadas}</div><div class='kpi-label'>Backlog da Área</div></div>
            </div>
        """, unsafe_allow_html=True)
        
        col_ar1, col_ar2 = st.columns(2)
        with col_ar1:
            st.markdown("<p style='font-size:0.8rem; color:#8b949e; text-transform:uppercase;'>Status Interno do Setor</p>", unsafe_allow_html=True)
            if not df_foco_unicas.empty:
                st_counts_ar = df_foco_unicas["status_execucao"].value_counts().reset_index()
                fig_ar1 = px.pie(st_counts_ar, values="count", names="status_execucao", hole=0.55, color="status_execucao", color_discrete_map=COLOR_MAP)
                fig_ar1.update_layout(**layout_comum_github)
                st.plotly_chart(fig_ar1, use_container_width=True, key="graph_ar_pie_status")
         
        with col_ar2:
            st.markdown("<p style='font-size:0.8rem; color:#8b949e; text-transform:uppercase;'>Distribuição de Especialidades</p>", unsafe_allow_html=True)
            if not df_foco_unicas.empty:
                disc_counts_ar = df_foco_unicas["disciplina"].value_counts().reset_index()
                fig_ar2 = px.pie(disc_counts_ar, values="count", names="disciplina", hole=0.55, color="disciplina", color_discrete_map=DISCIPLINA_CORES)
                fig_ar2.update_layout(**layout_comum_github)
                st.plotly_chart(fig_ar2, use_container_width=True, key="graph_ar_pie_disc")

        st.markdown("<div style='margin: 15px 0; border-bottom: 1px solid #30363d;'></div>", unsafe_allow_html=True)
        st.markdown("<h4 style='font-size:1.0rem; font-weight:500; margin-bottom:10px; color:#f0f6fc;'>📡 Telemetria e Avanço por Disciplina Industrial</h4>", unsafe_allow_html=True)
        
        disciplinas_existentes = sorted(list(df_foco_unicas["disciplina"].dropna().unique()))
        if disciplinas_existentes:
            disc_selecionada_telemetria = st.segmented_control(
                "Selecione a Especialidade para Monitorar Avanço Coletivo:", 
                options=disciplinas_existentes, 
                default=disciplinas_existentes[0],
                key="telemetry_discipline_select"
            )
            
            if disc_selecionada_telemetria:
                df_telemetria_disc = df_foco_unicas[df_foco_unicas["disciplina"] == disc_selecionada_telemetria]
                tot_t = len(df_telemetria_disc)
                rec_t = len(df_telemetria_disc[df_telemetria_disc["status_execucao"] == "Realizada"])
                pct_t = (rec_t / tot_t * 100) if tot_t > 0 else 0.0
                
                st.markdown(f"""
                    <div class='telemetry-box'>
                        <div class='telemetry-header'>
                            <span style='font-size:0.9rem; font-weight:600; color:#58a6ff;'>Especialidade: {disc_selecionada_telemetria}</span>
                            <span style='font-family:JetBrains Mono, monospace; font-size:0.85rem; color:#8b949e;'>Carga Amostrada: {tot_t} OMS Ativas</span>
                        </div>
                        <div class='telemetry-progress-bg'>
                            <div class='telemetry-progress-fill' style='width: {pct_t}%;'></div>
                            <div class='telemetry-text-overlay'>{pct_t:.1f}% CONCLUÍDO ({rec_t} de {tot_t} ordens executadas)</div>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
        else:
            st.caption("Sem disciplinas registradas para computar telemetria nesta área.")
    ...
    with aba_exec:
        ...
        area_sel_exec = st.selectbox("Filtrar Visão por Área:", lista_areas_disponiveis, key="area_exec_v19")
        ...
        exec_sel = st.selectbox("Filtrar por técnico de campo:", lista_executantes, key="c_exec_v19", label_visibility="collapsed")
        ...
        if exec_sel != "Selecione o profissional...":
            ...
            with col_pie1:
                ...
                f_pie_exec.update_layout(**layout_comum_github, height=200)
            ...
            with col_pie2:
                ...
                f_pie_horas.update_layout(**layout_comum_github, height=200)
    ...
    with aba_disc:
        ...
        area_sel_disc = st.selectbox("Filtrar Linha por Área:", lista_areas_disponiveis, key="area_disc_v19")
        df_foco = df_geral[df_geral["area"].str.strip() == area_sel_disc].copy()
        disc_sel = st.selectbox("Selecione a especialidade técnica:", sorted(list(df_foco["disciplina"].dropna().unique())), key="c_disc_page_v19")
                ...
                for eng_day, pt_day in dias_mapeamento.items():
                    ...
                    if not df_dia_disc.empty:
                        ...
                        with st.expander(f"📅 {pt_day} — {disc_sel} ({qtd_unicas_disc} Atividades)", expanded=expandir_se_hoje):
                            render_cards_operacionais(df_dia_disc, f"disc_crd_v19_{disc_sel}_{eng_day}")

    # RODAPÉ PRINCIPAL
    st.markdown("<div style='margin-top:20px; font-size:0.75rem; color:#8b949e;'><center>📊 Sistema Integrado de Operações • Visão Global Planta Guaíba</center></div>", unsafe_allow_html=True)
else:
    st.markdown("""
