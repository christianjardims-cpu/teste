...
if "versao_layout" not in st.session_state or st.session_state.versao_layout != "16.2_total_cts_fields":
    st.session_state.clear()  
    st.session_state.versao_layout = "16.2_total_cts_fields"

# Cores de mapeamento de status
COLOR_MAP = {
    "Realizada": "#2ea44f",
    "Pendente": "#f85149",
...
# Paleta expandida de disciplinas para contemplar AUTOMAÇÃO, LUBRIFICAÇÃO, INSPEÇÃO, etc.
DISCIPLINA_CORES = {
    "Mecânica": "#1f6feb",
    "Elétrica": "#bc8cff",
...
<style>
    ...
    .ios-clock-widget:hover { 
        border-color: #58a6ff;
        box-shadow: 0 6px 22px rgba(56, 139, 253, 0.12);
        transform: translateY(-2px);
    }
    ...
    /* Grid de Informações Adicionais no Card */
    .card-meta-grid {
        display: grid;
    ...
    .meta-item { font-size: 0.78rem; color: #8b949e; }
    ...
    .kpi-card:hover { 
        border-color: #58a6ff;
        box-shadow: 0 4px 12px rgba(56, 139, 253, 0.08);
    }
    ...
    /* Estilização Customizada da Assinatura no Menu Lateral */
    .sidebar-signature { 
        text-align: center; 
        margin-top: 35px; 
        padding-top: 20px; 
    ...
    .side-corp-title { font-size: 1.1rem; font-weight: 700; color: #f0f6fc; letter-spacing: 1px; margin: 0; }
    .side-corp-author { font-size: 0.72rem; color: #8b949e; margin-top: 4px; font-family: 'JetBrains Mono', monospace; line-height: 1.4; }
</style>
...
# -----------------------------------------------------------------------------
# 2. SISTEMA DE BANCO DE DADOS (SQLITE OTIMIZADO COM CAMPOS ADICIONAIS)
# -----------------------------------------------------------------------------
def inicializar_banco():
    with sqlite3.connect(DB_NOME) as conn:
        cursor = conn.cursor()
        # Tabela atualizada com suporte estruturado para novos dados coletados
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS programacao (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ordem TEXT, area TEXT, subarea TEXT, descricao TEXT, operacao TEXT, 
                executante TEXT, data_inicio TEXT, tempo_execucao TEXT, 
                disciplina TEXT, status_execucao TEXT, comentario TEXT,
                descricao_ct TEXT, local_instalacao TEXT, planejador TEXT
            )
        """)
...
def atualizar_banco_completo(df_novo):
    with sqlite3.connect(DB_NOME) as conn:
        ...
        for _, row in df_novo.iterrows():
            # Extração dinâmica de CT inteligente mapeando Automação e demais frentes
            ct_bruto = str(row.get("Centro de Trabalho Op.", "")).upper()
            
            if "E" in ct_bruto:
                disc = "Elétrica"
            elif "I" in ct_bruto or "INST" in ct_bruto:
                disc = "Instrumentação"
            elif "AUT" in ct_bruto or "AUTO" in ct_bruto:
                disc = "Automação"
            elif "LUB" in ct_bruto or "LUBR" in ct_bruto:
                disc = "Lubrificação"
            elif "CALD" in ct_bruto or "CALV" in ct_bruto:
                disc = "Caldeiraria"
            elif "INSP" in ct_bruto:
                disc = "Inspeção"
            elif "CIV" in ct_bruto:
                disc = "Civil"
            elif "M" in ct_bruto or "MEC" in ct_bruto:
                disc = "Mecânica"
            else:
                disc = "Outros"

            cursor.execute("""
                INSERT INTO programacao (ordem, area, subarea, descricao, operacao, executante, data_inicio, tempo_execucao, disciplina, status_execucao, comentario, descricao_ct, local_instalacao, planejador)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                str(row.get("Ordem", "")), 
                str(row.get("Área", row.get("Centro de custo", "Guaíba"))), 
                str(row.get("Subárea", row.get("Sub-área", "N/A"))),
                str(row.get("Descrição da Ordem", row.get("Descrição", ""))),
                str(row.get("Texto Breve da Operação", row.get("Operação", ""))), 
                str(row.get("Executante", "Não Atribuído")), 
                str(row.get("Data de Início", row.get("Início", ""))),
                str(row.get("Tempo de Execução", row.get("Duração", "4h"))), 
                disc,
                str(row.get("Status_Execucao", "Pendente")), 
                str(row.get("Comentario", "")),
                str(row.get("Descrição do CT", row.get("Descrição do centro de trabalho", "CT Geral"))),
                str(row.get("Local de Instalação", row.get("Local de instalação", "Instalação Geral"))),
                str(row.get("Planejador", "Planejamento Central"))
            ))
        conn.commit()
...
# -----------------------------------------------------------------------------
# 4. CONFIGURAÇÕES INTERNES DE DATA E JORNADA
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5. SIDEBAR: FLUID DESIGN WITH PERMANENT FOOTER SIGNATURE
# -----------------------------------------------------------------------------
with st.sidebar:
    ...
    # ASSINATURA EXCLUSIVA DO DEV CHRISTIAN JARDIM FIXADA NA BASE DA BARRA LATERAL
    st.markdown("""
        <div class='sidebar-signature'>
            <p class='side-corp-title'>CMPC</p>
            <p class='side-corp-author'>Design Dev<br>Christian Jardim</p>
            <small style='font-size:0.68rem; color:#8b949e; display:block; margin-top:8px;'>📊 Monitoramento Global • Unidades CMPC</small>
        </div>
    """, unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# 6. HEADER DESIGN CONTEMPORÂNEO (CLEAN OVERVIEW)
# -----------------------------------------------------------------------------
col_tit1, col_tit2 = st.columns([2.3, 1.7])
with col_tit1:
    ...
    st.markdown("<p style='color: #8b949e; margin: 0; font-size: 1rem;'>Manutenção Integrada • Monitoramento e Controle Global de Ativos</p>", unsafe_allow_html=True)
with col_tit2:
    st.markdown(f"""
...
# -----------------------------------------------------------------------------
# 7. FILTROS E RENDERIZAÇÃO COMPLETA DOS CARDS COM NOVOS METADADOS
# -----------------------------------------------------------------------------
...
def render_cards_operacionais(sub_df, unique_prefix):
    ...
    for _, row in sub_df.iterrows():
        ...
        # Coleta segura dos novos campos solicitados
        area_field = row.get("area", "N/A")
        subarea_field = row.get("subarea", "N/A")
        ct_desc_field = row.get("descricao_ct", "CT Geral")
        local_field = row.get("local_instalacao", "Instalação Geral")
        planejador_field = row.get("planejador", "Planejamento")
        ...
        if modo_exibicao == "Cards":
            st.markdown(f"""
                <div class='os-card' style='background: {bg_card}; border-left: 4px solid {border_left_color};'>
                    ...
                    <div style='font-size:0.78rem; color:#8b949e; display:flex; gap:12px; margin-bottom: 4px;'>
                        <span><b>🛠️ Op:</b> {operacao[:60]}...</span>
                        <span><b>👤 Executante:</b> <span style='color: #c9d1d9;'>{row['executante']}</span></span>
                    </div>
                    
                    <div class='card-meta-grid'>
                        <div class='meta-item'>🏭 <b>Área:</b> {area_field}</div>
                        <div class='meta-item'>🌿 <b>Subárea:</b> {subarea_field}</div>
                        <div class='meta-item'>🔧 <b>Desc. CT:</b> {ct_desc_field}</div>
                        <div class='meta-item'>📍 <b>Local Inst.:</b> {local_field}</div>
                        <div class='meta-item'>📋 <b>Planejador:</b> {planejador_field}</div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"📌 <span style='border-left: 3px solid {border_left_color}; padding-left:5px;'><b>OMS {ordem}</b></span> - <span style='color:{cor_disc}; font-weight:600;'>{row['disciplina']}</span> | <span style='color:#f0f6fc;'>{desc[:60]}...</span> [<b>{area_field}</b> - {subarea_field}]", unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
# 8. SISTEMA DE ABAS EM ALTA DENSIDADE (CONTEMPLANDO TODOS OS CTS)
# -----------------------------------------------------------------------------
if st.session_state.db_data is not None and not st.session_state.db_data.empty:
    ...
    aba_fabrica, aba_geral, aba_exec, aba_disc = st.tabs([
        ...
        "⚙️ Visão por Disciplina (Todos os CTs)"
    ])
    
    # -------------------------------------------------------------------------
    # ABA 1: VISÃO GERAL DA FÁBRICA (CONSOLIDADO COMPLETO)
    # -------------------------------------------------------------------------
    with aba_fabrica:
        st.markdown("<h3 style='font-size:1.2rem; font-weight:500; margin-bottom:12px;'>Métricas Consolidadas de Toda a Unidade</h3>", unsafe_allow_html=True)
        ...
        with col_f1:
            st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Status Operacional Global</p>", unsafe_allow_html=True)
            if not df_fabrica_unicas.empty:
                st_counts_fabrica = df_fabrica_unicas["status_execucao"].value_counts().reset_index()
                fig_f1 = px.pie(st_counts_fabrica, values="count", names="status_execucao", hole=0.6, color="status_execucao", color_discrete_map=COLOR_MAP)
                fig_f1.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(t=10,b=10,l=10,r=10), showlegend=True)
                st.plotly_chart(fig_f1, use_container_width=True, key="graph_fabrica_status")
        
        with col_f2:
            st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Volume de Trabalho por Disciplina (Mapeamento Completo de CTs)</p>", unsafe_allow_html=True)
            if not df_fabrica_unicas.empty:
                disc_counts_fabrica = df_fabrica_unicas["disciplina"].value_counts().reset_index()
                disc_counts_fabrica.columns = ["Disciplina", "Quantidade"]
                disc_counts_fabrica = disc_counts_fabrica.sort_values(by="Quantidade", ascending=False)
                ...
                fig_f2 = px.bar(
                    ...
                    text_auto=True, 
                    color="Disciplina", 
                    color_discrete_map=DISCIPLINA_CORES
                )
                fig_f2.update_layout(
                    ...
                    showlegend=False, 
                    xaxis_title="Centros de Trabalho Identificados", 
                    yaxis_title="Quantidade de Ordens"
                )
                st.plotly_chart(fig_f2, use_container_width=True, key="graph_fabrica_disciplinas")
                
        st.markdown("<div style='margin: 15px 0; border-bottom: 1px solid #30363d;'></div>", unsafe_allow_html=True)
        st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Acompanhamento Geral de Ordens Ativas</p>", unsafe_allow_html=True)
        render_cards_operacionais(df_geral, "fabrica_completo")

    # -------------------------------------------------------------------------
    # ABA 2: VISÃO POR ÁREA
    # -------------------------------------------------------------------------
    with aba_geral:
        ...
        with col_g2:
            st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Atividades por Disciplina Cadastrada (Mapeamento Abrangente)</p>", unsafe_allow_html=True)
            if not df_foco_unicas.empty:
        ...
        st.markdown("<p style='font-size:0.85rem; color:#8b949e; text-transform:uppercase; font-weight:600; letter-spacing:0.5px;'>Análise de Entrega Concluída por CT (%)</p>", unsafe_allow_html=True)
        if not df_foco_unicas.empty:
            ...
            if df_exec["data_parsed"].isna().all():
                ...
                for eng_day, pt_day in dias_mapeamento.items():
                    ...
                    if not df_dia_specifico.empty:
                        qtd_unicas = df_dia_specifico["ordem"].nunique()
    ...
    # -------------------------------------------------------------------------
    # ABA 4: VISÃO POR DISCIPLINA (CRONOGRAMA TÁTICO EXPANDIDO)
    # -------------------------------------------------------------------------
    with aba_disc:
        ...
        # Selectbox preenchida com todas as disciplinas detectadas (Automação, Elétrica, etc.)
